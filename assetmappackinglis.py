...
def givePackingList(folderpath,uuidcommon):
    ...
    #Annotation text
    ET.SubElement(doc,"AnnotationText").text ="Agit ssets of "+Path(path).parts[-1]
...
# to gothrough all the files in a folder
    ...
    for root, dirs, files in os.walk(folderpath):
        for filename in files:
            ...
            ET.SubElement(Asset,"Hash").text =findHashValue(file_path)
...
def giveAssetMap(folderpath,uuidcommon):
...
if __name__ == '__main__':
    ...
    #to check wheather the directory is empty or not
        if len(os.listdir(path)) == 0:
            ...
            givePackingList(folderpath,uuidcommon)
            giveAssetMap(folderpath,uuidcommon)
